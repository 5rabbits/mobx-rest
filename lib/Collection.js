...
var Collection = (_class = function (_Base) {
  ...
  (0, _createClass3.default)(Collection, [{
    ...
    value: function add(data) {
      ...
      (_models = this.models).push.apply(_models, (0, _toConsumableArray3.default)(data.map(function (attributes) {
        var model = _this2.build(attributes);

        _this2.addToModelCollections(model);

        return model;
      })));
    }
  }, {
    key: 'addToModelCollections',
    value: function addToModelCollections(model) {
    ...
    value: function reset(data) {
      ...
      this.models.replace(data.map(function (attributes) {
        var model = _this3.build(attributes);

        _this3.addToModelCollections(model);
    ...
    value: function remove(ids) {
      ...
      ids.forEach(function (id) {
        ...
        if (id instanceof _Model2.default && id.collections.indexOf(_this4) !== -1) {
          model = id;
        ...
        _this4.models.splice(_this4.models.indexOf(model), 1);
        model.collections.splice(model.collections.indexOf(_this4), 1);
      });
    }
    ...
    value: function set(resources) {
      ...
      resources.forEach(function (resource) {
        var model = _this5.get(resource.id);

        if (model && change) {
          model.set(resource instanceof _Model2.default ? resource.toJS() : resource);
        }

        if (!model && add) _this5.add([resource]);
      });
    }
    ...
    key: 'create',
    value: function create(attributesOrModel) {
      var _this6 = this;

      var _ref4 = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {},
          _ref4$optimistic = _ref4.optimistic,
          optimistic = _ref4$optimistic === undefined ? true : _ref4$optimistic;

      var model = this.build(attributesOrModel);

      this.addToModelCollections(model);

      var _model$save = model.save(),
          abort = _model$save.abort,
          promise = _model$save.promise;
